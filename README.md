...
## 프로젝트 소개 및 주제
+ 프로젝트 명인 ***maturi는 맛집 + 집들이의 합성어***로 집들이 하며 집을 소개하듯이 리뷰글을 통해 ***맛집을 소개***하자는 의미입니다.
+ 전반적인 웹의 기본 소양이 되는 ***게시판, 회원 CRUD를 중심***으로 한 ***커뮤니티 사이트***입니다.
+ 카카오 map api, 카카오 로그인 api, 네이버 로그인 api 등의 ***다양한 api 통신을 적극 활용***하였습니다.
+ ***많은 경우의 수가 존재하는 검색 조건, 정렬***을 ***하나의 동적 쿼리문***으로 처리하여 차별화하였습니다.
+ ***Docker를 통해 컨테이너화된 애플리케이션***을 구축하고, ***AWS의 클라우드 서비스***들을 활용하여 안전하고 확장 가능한 인프라를 설계 및 관리하는 경험을 쌓았습니다. 
...
## 기술 스택
 * 개발 및 마크업 언어 : JAVA(Oracle OpenJDB version 1.8), Html + Css, Javascript </br>
